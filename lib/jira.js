...
var JiraApi = exports.JiraApi = function(protocol, host, port, username, password, apiVersion, verbose, strictSSL) {
    ...
    // Default options for request (optionally extended later)
    this.defaultOptions = {};
...
(function() {
    ...
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#d2e3053)
    /* Example response
     * [
     *   {
     ...
     * ]
     */
    this.listResolutions = function(callback) {

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/resolution'),
            method: 'GET'
        };
        ...
        this.request(options, function(error, response, body) {
            ...
            if (response.statusCode !== 200) {
                callback(response.statusCode + ': Unable to connect to JIRA during getResolutions.');
                return;
            }
    ...
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#d2e1936)
    /* Example response
     * [
     *   {
     ...
     * ]
     */
    this.listStatuses = function(callback) {
        ...
        this.request(options, function(error, response, body) {
            ...
            if (response.statusCode !== 200) {
                callback(response.statusCode + ': Unable to connect to JIRA during getStatuses.');
                return;
            }
    ...
    // ## Get the create meta for issues ##
    // ### Takes ###
    //
    // *  projects: array of project keys to query. Empty array will query all.
    // *  issuetype: name of issuetype to query. null will query all.
    // *  expand: array of entities to be expanded. Empty array will use server default: projects.issuetypes.
    // *  callback: for when it's done
    //
    // ### Returns ###
    //
    // *  error: string of the error
    // *  meta: an object of the issue metadata
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id2e1477)
    this.getIssueMeta = function(projects, issuetype, expand, callback) {

        var params = {};
        if (projects && projects.length > 0) params.projectKeys =  projects.join(',');
        if (issuetype && issuetype.length)   params.issuetypeNames = issuetype;
        if (expand && expand.length > 0)     params.expand = expand.join(',');

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/issue/createmeta'),
            qs: params,
            method: 'GET'
        };
        ...
        this.request(options, function(error, response, body) {
            ...
            if (response.statusCode !== 200) {
                callback(response.statusCode + ': Unable to connect to JIRA during getIssueMeta.');
                return;
            }
    ...
    // ## Get types of issue create meta ##
    // ### Takes ###
    //
    // *  projects: array of project keys to query. Empty array will query all.
    // *  callback: for when it's done
    ...
    //
    // *  error: string of the error
    // *  meta: an object of the issue types
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id2e1477)
    /*
     ...
     *     {
     *     "avatarUrls": {
     *       "16x16": "http://localhost:8090/secure/projectavatar?size=xsmall&pid=10000&avatarId=10011",
     *       "24x24": "http://localhost:8090/secure/projectavatar?size=small&pid=10000&avatarId=10011",
     *       "32x32": "http://localhost:8090/secure/projectavatar?size=medium&pid=10000&avatarId=10011",
     *       "48x48": "http://localhost:8090/secure/projectavatar?pid=10000&avatarId=10011"
     *     },
     *     "id": "10000",
     *     "issuetypes": [
     *       {
     *       "description": "The sub-task of the issue",
     *       "iconUrl": "http://localhost:8090/images/icons/issuetypes/subtask_alternate.png",
     *       "id": "5",
     *       "name": "Sub-task",
     *       "self": "http://localhost:8090/rest/api/2/issuetype/5",
     *       "subtask": true
     *     },
     *     {
     *       "description": "A new feature of the product, which has yet to be developed.",
     *       "iconUrl": "http://localhost:8090/images/icons/issuetypes/newfeature.png",
     *       "id": "2",
     *       "name": "New Feature",
     *       "self": "http://localhost:8090/rest/api/2/issuetype/2",
     *       "subtask": false
     *     },
     *     {
     *       "description": "A task that needs to be done.",
     *       "iconUrl": "http://localhost:8090/images/icons/issuetypes/task.png",
     *       "id": "3",
     *       "name": "Task",
     *       "self": "http://localhost:8090/rest/api/2/issuetype/3",
     *       "subtask": false
     *     }
     *     ],
     *     "key": "TEST",
     ...
     * }
     */
    this.getIssueMetaTypes = function(projects, callback) {
        this.getIssueMeta(projects, null, ['projects.issuetypes'], callback);
    };

    // ## Get fields from issue create meta ##
    // ### Takes ###
    //
    // *  projects: array of project keys to query. Empty array will query all.
    // *  issuetype: name of issuetype to query. null will query all.
    ...
    //
    // *  error: string of the error
    // *  meta: an object of the issue types, including fields
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id2e1477)
    this.getIssueMetaFields = function(projects, issuetype, callback) {
        this.getIssueMeta(projects, issuetype, ['projects.issuetypes.fields'], callback);
    };
...
// [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id290791)
    ...
    // If signature_method is not specified, it defaults to 'RSA-SHA1'.
    //
    // ### Takes ###
    //
    // *  oauth: oauth object as specified by the request module
    //
    // ### Returns ###
    //
    // [Request Doc](https://github.com/mikeal/request#oauth-signing)
    /*
     * Format of oauth argument:
     * {
     ...
     * }
     */
    this.useOAuth = function(oauth) {
      if (oauth.signature_method === undefined)
        oauth.signature_method = 'RSA-SHA1';

      this.defaultOptions.oauth = oauth;
      this.request = require('request').defaults(this.defaultOptions);
    };
    ...
    // ## List my permissions for a project ##
    // ### Takes ###
    //
    // *  project: key of project to scope returned permissions for.
    // *  callback: for when it's done
    //
    // ### Returns ###
    // *  error string
     ...
     * }
     */
    this.myProjectPermissions = function(project, callback) {
        var params = {};
        params.projectKey = project;

        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/mypermissions'),
            qs: params,
            method: 'GET',
            json: true
        };
        ...
        this.request(options, function(error, response, body) {
            ...
            callback(response.statusCode + ': Error while retrieving permissions for project');
